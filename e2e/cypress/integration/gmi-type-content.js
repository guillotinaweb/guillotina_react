...
describe('test GMI type', function () {
  ...
  it('creates a GMI item as Admin, modifies it and delete it', function () {
    ...
    // Modify Item
    cy.get(`[data-test='${ITEMS_PANELS_SELECTORS.prefixItem}-test-gmi-item']`).click()
    cy.wait('@canido')
    ...
    cy.wait('@patch')
    cy.get(NOTIFICATION_SELECTOR).should('contain', `Field boolean_field, updated!`)
    ...
    cy.wait('@patch')
    cy.get(NOTIFICATION_SELECTOR).should('contain', `Field choice_field, updated!`)
    ...
    cy.get(
      ...
      cy.get(EDITABLE_FORM_SELECTORS.field).clear().type('first item').type('{enter}')
      cy.get(EDITABLE_FORM_SELECTORS.field).clear().type('second item').type('{enter}')
      cy.get(EDITABLE_FORM_SELECTORS.btnSave).click()
    })
    cy.wait('@patch')
    cy.get(NOTIFICATION_SELECTOR).should('contain', `Field list_field, updated!`)
